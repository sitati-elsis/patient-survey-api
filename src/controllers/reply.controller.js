...
const createReply = catchAsync(async (req, res) => {
  ...
  const patient = surveyResponseTokenDoc.metadata.patient;
  const reply = await replyService.createReply(campaignId, req.body, patient);
...
const updateReply = catchAsync(async (req, res) => {
  const reply = await replyService.updateReplyById(
    req.params.replyId,
    req.body
  );
  res.send(reply);
